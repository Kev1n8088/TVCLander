...
class SCH1 {
    ...
    uint8_t cs_p;
    uint32_t freq;
    SPIClass *spi; ///< Pointer to SPI bus interface
    // Adafruit_SPIDevice *spi_dev = NULL; ///< Pointer to SPI bus interface

public:
    SCH1(uint8_t rst_pin);
    int         SCH1_init(uint8_t cs_pin, SCH1_filter sFilter, SCH1_sensitivity sSensitivity, SCH1_decimation sDecimation, bool enableDRY,
                                uint32_t frequency = 10000000, SPIClass *theSPI = &SPI);

...
size_t LINK80::packCommandAck(uint8_t command_type, uint8_t command_id, uint8_t* p, uint32_t vehicle_ms, uint32_t down_count) {
    uint8_t payload[11];
    size_t offset = 0;
...
float LINK80::packUINT32(uint8_t* buffer, size_t offset, uint32_t value) {

...
SCH1::SCH1(uint8_t rst_pin) {
    // Constructor for SCH1 class, initializes the reset pin.
    // This pin is used to reset the sensor if needed.
    this->rst_p = rst_pin;
    pinMode(rst_p, OUTPUT);
    digitalWrite(rst_p, HIGH); // Set the reset pin to high initially
}
 ...
 * @return None                           
 */
void SCH1::SCH1_reset(void) {
    // Reset the SCH1 sensor by toggling the reset pin.
    // This function should be called to reset the sensor to its initial state.
 ...
 * @return None                           
 */
void SCH1::SCH1_sendSPIreset(void)
{
 ...
 *         false = invalid
 */
bool SCH1::SCH1_isValidFilterFreq(uint32_t Freq)
{   
 ...
 *         false = invalid                          
 */
bool SCH1::SCH1_isValidRateSens(uint32_t Sens)
{   
 ...
 *         false = invalid                          
 */
bool SCH1::SCH1_isValidAccSens(uint32_t Sens)
{   
 ...
 *         false = invalid                          
 */
bool SCH1::SCH1_isValidDecimation(uint32_t Decimation)
{   
 ...
 *         false = invalid                          
 */
bool SCH1::SCH1_isValidSampleRate(uint32_t Freq)
{   
 ...
 */
int SCH1::SCH1_setFilters(uint32_t Freq_Rate12, uint32_t Freq_Acc12, uint32_t Freq_Acc3)
{
    uint32_t dataField;
 ...
 *         SCH1_ERR_* = failure. Please see header file for error definitions.                      
 */
int SCH1::SCH1_setRateSensDec(uint16_t Sens_Rate1, uint16_t Sens_Rate2, uint16_t Dec_Rate2)
{
    uint32_t dataField;
    uint32_t bitField;
 ...
 *         SCH1_ERR_* = failure. Please see header file for error definitions.                      
 */
int SCH1::SCH1_getRateSensDec(uint16_t *Sens_Rate1, uint16_t *Sens_Rate2, uint16_t *Dec_Rate2)
{
    uint32_t dataField;
    uint64_t responseFrame_Rate_Ctrl;
 ...
 *         SCH1_ERR_* = failure. Please see header file for error definitions.                      
 */
int SCH1::SCH1_setAccSensDec(uint16_t Sens_Acc1, uint16_t Sens_Acc2, uint16_t Sens_Acc3, uint16_t Dec_Acc2)
{
    uint32_t dataField;
    uint32_t bitField;
 ...
 *         SCH1_ERR_* = failure. Please see header file for error definitions.                      
 */
int SCH1::SCH1_getAccSensDec(uint16_t *Sens_Acc1, uint16_t *Sens_Acc2, uint16_t *Sens_Acc3, uint16_t *Dec_Acc2)
{
    uint32_t dataField;
    uint64_t responseFrame_Acc12_Ctrl;
    uint64_t responseFrame_Acc3_Ctrl;
 ...
 *         SCH1_ERR_* = failure. Please see header file for error definitions.                      
 */
int SCH1::SCH1_enableMeas(bool enableSensor, bool setEOI)
{
 ...
 *         SCH1_ERR_* = failure. Please see header file for error definitions.                      
 */
int SCH1::SCH1_setDRY(int8_t polarity, bool enable)
{
 ...
 */
uint32_t SCH1::SCH1_convertFilterToBitfield(uint32_t Freq)
{
    switch (Freq)
    {
        case 13:
 ...
 */
uint32_t SCH1::SCH1_convertRateSensToBitfield(uint32_t Sens)
{
    switch (Sens)
    {
        case 1600:
 ...
 */
uint32_t SCH1::SCH1_convertBitfieldToRateSens(uint32_t bitfield)
{
    switch (bitfield)
    {
        case 0x02:          // 010
 ...
 * @return Equivalent bit field for building the rate sensitivity setting SPI-frame.                   
 */
uint32_t SCH1::SCH1_convertAccSensToBitfield(uint32_t Sens)
{
    switch (Sens)
    {
        case 3200:
 ...
 */
uint32_t SCH1::SCH1_convertBitfieldToAccSens(uint32_t bitfield)
{
    switch (bitfield)
    {
        case 0x01:          // 001
 ...
 */
uint32_t SCH1::SCH1_convertDecimationToBitfield(uint32_t Decimation)
{
    switch (Decimation)
    {
        case 2:
 ...
 */
uint32_t SCH1::SCH1_convertBitfieldToDecimation(uint32_t bitfield)
{
    switch (bitfield)
    {
        case 0x00:      // 001
 ...
 *
 * @return SCH1_OK = success, SCH1_MT_ERR_* = failure. Please see header file for error definitions.
 */
int SCH1::SCH1_getStatus(SCH1_status *Status)
{
    if (Status == NULL) {
 ...
 */
bool SCH1::SCH1_verifyStatus(SCH1_status *Status)
{
    if (Status == NULL) {
        return SCH1_ERR_NULL_POINTER;
    }
 ...
 */
uint64_t SCH1::SCH1_sendRequest(uint64_t Request)
{
    uint64_t ReceivedData = 0;
    uint8_t txBuffer[6];
    uint8_t rxBuffer[6];
    uint8_t index;
    uint8_t size = 6;   // 48-bit SPI-transfer consists of six 8-bit transfers.
 ...
 *         false = error
 */
bool SCH1::SCH1_checkCRC8(uint64_t SPIframe)
{
 ...
 *         false = error
 */
bool SCH1::SCH1_checkCRC3(uint32_t SPIframe)
{
...
int SCH1::SCH1_init(uint8_t cs_pin, SCH1_filter sFilter, SCH1_sensitivity sSensitivity,
             SCH1_decimation sDecimation, bool enableDRY, uint32_t frequency, SPIClass *theSPI) {
    // Initialize the sensor with the provided filter, sensitivity, and decimation
    // settings. This function should set up the SPI device and configure the sensor.
    // Return SCH1_OK on success or an error code on failure.
    ...
    pinMode(cs_p, OUTPUT);
    digitalWrite(cs_p, HIGH); // Set CS pin high to deselect the sensor
    ...
    for(startup_attempt = 0; startup_attempt < 10; startup_attempt++){
        delay(35);
        ...
        // Write EN_SENSOR = 1
        if(enableDRY){
            SCH1_setDRY(0, true); // Set DRY active high and enabled
        } else {
            SCH1_setDRY(0, false); // Set DRY disabled
        }

        SCH1_enableMeas(true, false); // Enable sensor and set EOI bit
        delay(215);

        SCH1_getStatus(&SCH1statusAll);

        SCH1_enableMeas(true, true); // Enable sensor and set EOI bit, end of initialization
        ...
        if (!SCH1_verifyStatus(&SCH1statusAll)) {
            SCH1status = false;            
            Serial.print("SCH1 init failed, attempt: ");
            Serial.println(startup_attempt + 1);
 ...
 * @return None                       
 */
void SCH1::SCH1_convert_data(SCH1_raw_data *data_in, SCH1_result *data_out)
{
 ...
 *         false = no error
 */
bool SCH1::SCH1_check_48bit_frame_error(uint64_t *data, int size)
{
